...
export default {
  name: "editBtn",
  props: ["buttonTitle", "formData", "axiosUrl", "responseDispatch", "refresh"],
  data() {
    return {
      showNotify: false,
    ...
    submit() {
      ...
      axios.defaults.headers.common["Authorization"] =
        ...
        (document.cookie.match("(^|;) ?token=([^;]*)(;|$)")
          ...
          : null);
      axios
        .post(this.axiosUrl + this.formData.id, data)
        .then(response => {
          ...
          if (response.data.status === 555) {
            this.$store.dispatch(this.responseDispatch, this.formData.id);
