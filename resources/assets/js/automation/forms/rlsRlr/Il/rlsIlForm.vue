...
export default {
  ...
  data() {
    return {
      ...
      window: {
        headerid: "rlsIlForm",
        title: "ارجاع نامه داخلی",
              titleLine: "کارتابل/ ارجاعات  / ارجاع نامه داخلی",
      },
      submit: {
        buttonTitle: "ارسال",
        axiosUrl: "/api/v1/rlsIlRegister",
      },
      data: {
        id: "",
        note: "",
  ...
  computed: {
    ...
    lastId: {
      get() {
        return (this.data.id = this.$store.state.IlsIlrStore.selectIlr.id);
      },
    },
    lastSummary: {
      get() {
