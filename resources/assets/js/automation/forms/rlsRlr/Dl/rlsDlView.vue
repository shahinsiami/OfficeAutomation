...
export default {
  ...
  data() {
    return {
      ...
      window: {
        ...
        titleLine: "کارتابل/ ارجاعات  / مشاهده پیش نویس ارجاعی",
      },
      submit: {
        buttonTitle: "ثبت",
  ...
  methods: {
    ...
    openRefferal() {
      this.$store.dispatch( "selectDls", this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.id );
  ...
  computed: {
    lastId: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl) {
          return this.$store.state.RefferalStore.SelectRlsDl.id;
        } else {
          return "";
        }
      },
    ...
    lastSender: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl) {
          if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
            return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.sender;
          }
        }
      },
    },
    last_letter_priority: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl) {
          if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
            return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls
              .letter_priority;
          }
        }
      },
    ...
    last_security: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl) {
          if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
            return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.security;
          }
        }
      },
    },
    ...
    lastSummary: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl) {
          if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
            if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.summary) {
              return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.summary
                .summary;
            } else {
              return "";
            }
    ...
    lastContext: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl) {
          if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
            if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.context) {
              return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.context
                .content;
            } else {
              return "";
            }
    ...
    lastDocument: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
          return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.document;
        }
      },
    },
    lastReceiver: {
      get() {
        if (this.$store.state.RefferalStore.SelectRlsDl.dlr_dls) {
          return this.$store.state.RefferalStore.SelectRlsDl.dlr_dls.receiver;
        }
      },
    },
  },
